...
| KOPS_CLUSTER_NAME                                  | Cluster base hostname (E.g. `us-west-2.staging.ryanjarv.sh`)                                    |
...
| KOPS_STATE_STORE                                   | S3 Bucket that will be used to store the cluster state (E.g. `us-west-2.staging.ryanjarv.sh`)   |
...
```
✓   (rjsh-staging-admin) kops ⨠  kops validate cluster
Validating cluster us-west-2.staging.ryanjarv.sh
...
Your cluster us-west-2.staging.ryanjarv.sh is ready
```
...
```
✓   (rjsh-staging-admin) kops ⨠  kubectl get nodes
...
```
✓   (rjsh-staging-admin) backing-services ⨠  kubectl get pods --all-namespaces
...
3. Run `geodesic` shell (`staging.ryanjarv.sh`), assume role (`assume-role`) and change directory to `/conf/kops` folder
